...
STUDENTS_DATASET_PATH = "data/datasets/students-dataset/"
...
class loadDataset(data.Dataset):
    ...
    def normalize(self):
        ...
        for key, value in self.mean_std.items():
            mean = value[0]
            std = value[1]
            # change string to float and unk to 0
            self.dataframe[key] = self.dataframe[key].replace("unk", 0)
    ...
    def get_split(self, idx):
        ...
        numerical_data = self.numerical_data[idx]
        target_data = self.target_data[idx].reshape(-1).float()
        # get the correct subgroup indexes
        subgroup_indexes = []
        for group in self.subgroup_indexes:
            subgroup_indexes.append([i for i in group if i in idx])
        minority_subgroup = np.argmin([len(group) for group in subgroup_indexes])
        return IterableDataset(categorical_data, numerical_data, target_data, subgroup_indexes, minority_subgroup)
...
class IterableDataset:
    def __init__(self, cat, num, target, subgroup_indexes, subgroup_minority):
        """
...
def get_dataset_stats(dataset, train_or_test):
    ...
    elif dataset == "students-dataset":
        data_path = STUDENTS_DATASET_PATH
